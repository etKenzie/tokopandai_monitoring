...
const RepaymentRiskChart = ({ filters }: RepaymentRiskChartProps) => {
  ...
  const formatValue = (value: number) => {
    if (chartType === 'amounts' || chartType === 'profit') {
    ...
    if (chartType === 'amounts') {
      series = [
        ...
        },
        {
          ...
          data: months.map(month => chartData.monthly_data[month].total_unrecovered_repayment)
        }
      ];
    } else if (chartType === 'rates') {
      series = [
        {
          name: 'Repayment Recovery Rate',
          data: months.map(month => chartData.monthly_data[month].repayment_recovery_rate)
        }
      ];
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 2, mb: 3 }}>
          ...
          <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
            <FormControl size="small">
              ...
              >
                <MenuItem value="amounts">Repayment Amounts</MenuItem>
                <MenuItem value="rates">Recovery Rate</MenuItem>
                <MenuItem value="profit">Admin Fee Profit</MenuItem>
              </Select>
            </FormControl>
...
export default RepaymentRiskChart;
