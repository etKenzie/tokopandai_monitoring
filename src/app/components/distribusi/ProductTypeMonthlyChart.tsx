...
import { fetchProductTypeMonthly, ProductTypeMonthlyResponse } from '@/app/api/distribusi/DistribusiSlice';
import {
    Box,
    Card,
    CardContent,
...
interface ProductTypeMonthlyChartProps {
  filters: {
    agent: string;
    area: string;
    segment?: string;
...
const ProductTypeMonthlyChart = ({ filters }: ProductTypeMonthlyChartProps) => {
  ...
  const fetchChartData = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetchProductTypeMonthly({
        start_month: startMonthYear,
        end_month: endMonthYear,
  ...
  const prepareChartData = () => {
    ...
    // Get all unique product types across all months
    const allProductTypes = new Set<string>();
    months.forEach(month => {
      const monthData = chartData.data[month];
      if (monthData) {
        Object.keys(monthData).forEach(type => allProductTypes.add(type));
      }
    });
    
    const productTypes = Array.from(allProductTypes);
    ...
    if (chartType === 'amounts') {
      series = productTypes.map(productType => ({
        ...
        data: months.map(month => {
          const monthData = chartData.data[month];
          return monthData && monthData[productType] ? monthData[productType].total_invoice : 0;
        })
      }));
  ...
  const chartDataConfig = useMemo(() => {
    console.log('Recalculating chart data config...', { chartData, chartType });
...
export default ProductTypeMonthlyChart;
